...
const UserProfile = ({ Update }) => {
  ...
  const formikUserProfileData = useFormik({
    ...
    onSubmit: (values, { setSubmitting }) => {
      ...
      axios.patch('https://pro-themers-backend.onrender.com/users/update', formData, {
        headers: {
  ...
  const formikChangePasswordData = useFormik({
    ...
    onSubmit: (values, { setSubmitting }) => {
      ...
      axios.patch('https://pro-themers-backend.onrender.com/users/profile/change-pass', { currentpass: values.currentPassword, pass: values.newPassword, confirmpass: values.re_enterNewPassword }, { headers: { Authorization: Token } })
        .then((res) => {
          console.log(res);
          ...
          <Grid item md={4} xs={12}>
            <Item >
              <Stack alignItems="center" padding="5px" spacing={1.5}>
                <img src={`https://pro-themers-backend.onrender.com/images/${userData?.profilepic}`} alt={userData?.fname + '-img'} width={120} height={120} style={{ borderRadius: "50%" }} />
                <Box>
